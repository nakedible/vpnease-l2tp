...
#define L2TP_PPP_MODIFY_ARGS 																\
	...
	ARG(AUTH_NONE, 		"auth_none", 		0, 	bool,	"Allow unauthenticated PPP users. Default: NO"),				\
...
static int l2tp_parse_ppp_profile_arg(l2tp_ppp_arg_ids_t arg_id, struct cli_node *arg, char *arg_value, struct l2tp_api_ppp_profile_msg_data *msg)
{
	...
	case L2TP_PPP_ARGID_AUTH_PAP:
		L2TP_ACT_PARSE_ARG(arg, arg_value, ints[0], msg->flags, L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS);
		if (ints[0]) {
			msg->auth_flags &= ~L2TP_API_PPP_AUTH_REFUSE_PAP;
		} else {
			msg->auth_flags |= L2TP_API_PPP_AUTH_REFUSE_PAP;
		}
		msg->flags |= L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS;
		break;
	case L2TP_PPP_ARGID_AUTH_CHAP:
		L2TP_ACT_PARSE_ARG(arg, arg_value, ints[0], msg->flags, L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS);
		if (ints[0]) {
			msg->auth_flags &= ~L2TP_API_PPP_AUTH_REFUSE_CHAP;
		} else {
			msg->auth_flags |= L2TP_API_PPP_AUTH_REFUSE_CHAP;
		}
		msg->flags |= L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS;
	...
	case L2TP_PPP_ARGID_AUTH_MSCHAP:
		L2TP_ACT_PARSE_ARG(arg, arg_value, ints[0], msg->flags, L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS);
		if (ints[0]) {
			msg->auth_flags &= ~L2TP_API_PPP_AUTH_REFUSE_MSCHAP;
		} else {
			msg->auth_flags |= L2TP_API_PPP_AUTH_REFUSE_MSCHAP;
		}
		break;
	case L2TP_PPP_ARGID_AUTH_MSCHAPV2:
		L2TP_ACT_PARSE_ARG(arg, arg_value, ints[0], msg->flags, L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS);
		if (ints[0]) {
			msg->auth_flags &= ~L2TP_API_PPP_AUTH_REFUSE_MSCHAPV2;
		} else {
			msg->auth_flags |= L2TP_API_PPP_AUTH_REFUSE_MSCHAPV2;
		}
		msg->flags |= L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS;
	...
	case L2TP_PPP_ARGID_AUTH_EAP:
		L2TP_ACT_PARSE_ARG(arg, arg_value, ints[0], msg->flags, L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS);
		if (ints[0]) {
			msg->auth_flags &= ~L2TP_API_PPP_AUTH_REFUSE_EAP;
		} else {
			msg->auth_flags |= L2TP_API_PPP_AUTH_REFUSE_EAP;
		}
		msg->flags |= L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS;
	...
	case L2TP_PPP_ARGID_AUTH_NONE:
		L2TP_ACT_PARSE_ARG(arg, arg_value, ints[0], msg->flags, L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS);
		if (ints[0]) {
			msg->auth_flags |= L2TP_API_PPP_AUTH_NONE;
		} else {
			msg->auth_flags &= ~L2TP_API_PPP_AUTH_NONE;
		}
		msg->flags |= L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS;
...
static int l2tp_act_ppp_profile_show(struct cli_node *node, int argc, char *argv[], int *arg_num)
{
	...
	printf("  allowed authentications: %s%s%s%s%s%s\n", 
	       ...
	       clnt_res.auth_flags & L2TP_API_PPP_AUTH_NONE ? "NOAUTH " : "");
...
static void l2tp_config_dump_ppp_profile(FILE *file, struct l2tp_api_ppp_profile_msg_data *cfg)
{
 	...
 	if ((cfg->flags & (L2TP_API_PPP_PROFILE_FLAG_TRACE_FLAGS |
		...
		if (cfg->flags & L2TP_API_PPP_PROFILE_FLAG_AUTH_FLAGS) {
			if (cfg->auth_flags & L2TP_API_PPP_AUTH_REFUSE_PAP) {
				fprintf(file, "\tauth_pap=off \\\n");
			} else {
				fprintf(file, "\tauth_pap=on \\\n");
			}
			if (cfg->auth_flags & L2TP_API_PPP_AUTH_REFUSE_CHAP) {
				fprintf(file, "\tauth_chap=off \\\n");
			} else {
				fprintf(file, "\tauth_chap=on \\\n");
			}
			if (cfg->auth_flags & L2TP_API_PPP_AUTH_REFUSE_MSCHAP) {
				fprintf(file, "\tauth_mschapv1=off \\\n");
			} else {
				fprintf(file, "\tauth_mschapv1=on \\\n");
			}
			if (cfg->auth_flags & L2TP_API_PPP_AUTH_REFUSE_MSCHAPV2) {
				fprintf(file, "\tauth_mschapv2=off \\\n");
			} else {
				fprintf(file, "\tauth_mschapv2=on \\\n");
			}
			if (cfg->auth_flags & L2TP_API_PPP_AUTH_REFUSE_EAP) {
				fprintf(file, "\tauth_eap=off \\\n");
			} else {
				fprintf(file, "\tauth_eap=on \\\n");
			}
			if (cfg->auth_flags & L2TP_API_PPP_AUTH_NONE) {
				fprintf(file, "\tauth_none=yes \\\n");
			} else {
				fprintf(file, "\tauth_none=no \\\n");
			}
		}
