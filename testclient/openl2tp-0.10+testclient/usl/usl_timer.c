...
static unsigned int			usl_tick;
static int				usl_tick_pending;
...
static void usl_timer_tick_handler(int fd, void *arg)
{
	...
	usl_list_for_each(walk, tmp, &usl_timer_list) {
		...
		if (usl_tick > tim->expiry) {
			...
			/* timer expired */
			USL_DEBUG("%s: timer %p expired", __FUNCTION__, tim);
			...
			if (tim->interval > 0) {
				...
				USL_DEBUG("%s: timer %p restarting with interval=%d", __FUNCTION__, tim, tim->interval);
		       ...
		       usl_timer_interval_adjust_fn_t adjuster)
{
	...
	USL_DEBUG("%s: created timer %p expire %d", __FUNCTION__, tim, expires);
	return tim;
}
...
void usl_timer_delete(void *handle)
{
	...
	USL_DEBUG("%s: timer %p", __FUNCTION__, tim);
...
int usl_timer_is_running(void *handle)
{
	...
	USL_DEBUG("%s: %p", __FUNCTION__, tim);
...
void usl_timer_interval_set(void *handle, unsigned int interval)
{
	...
	USL_DEBUG("%s: %p, interval=%d", __FUNCTION__, tim, interval);
...
void usl_timer_expiry_set(void *handle, unsigned int expiry)
{
	...
	USL_DEBUG("%s: %p, expiry=%d", __FUNCTION__, tim, expiry);
