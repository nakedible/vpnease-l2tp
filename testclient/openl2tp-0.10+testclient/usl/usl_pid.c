...
int usl_pid_record_child(int pid, usl_pid_done_fn_t callback, void *arg)
{
	...
	USL_DEBUG("%s: pid=%d", __FUNCTION__, pid);
	return 0;
}

/* Called by application to remove the callback argument
 * from a registered child pid.
 */
void usl_pid_remove_callback_arg(int pid, void *arg)
{
	...
	usl_list_for_each(walk, tmp, &usl_child_list) {
		...
		if ((child->pid == pid) && (child->arg == arg)) {
			child->arg = NULL;
			break;
		}
	}
...
int usl_pid_reap_children(int waitfor)
{
	...
	while (((pid = waitpid(WAIT_ANY, &status, (waitfor ? 0: WNOHANG))) != -1) && (pid != 0)) {
		...
		if (have_callback && (child->callback != NULL)) {
			(*child->callback)(child->arg, sig);
		}

		if (have_callback) {
			USL_DEBUG("%s: pid %d reaped", __FUNCTION__, child->pid);
