...
struct l2tp_packet *l2tp_pkt_alloc(int num_bufs)
{
	...
	pkt = calloc(1, sizeof(*pkt) + (num_bufs * sizeof(struct l2tp_packet_buffer)));
	if (pkt == NULL) {
		goto out;
	}
...
void l2tp_pkt_free(struct l2tp_packet *pkt)
{
	...
	L2TP_DEBUG(L2TP_DATA, "%s: pkt=%p, msg=%d tid=%hu sid=%hu ns=%hu nr=%hu", __FUNCTION__,
		   pkt, pkt->msg_type, pkt->tunnel_id, pkt->session_id, pkt->nr, pkt->ns);

	if (pkt->num_bufs > 0) {
		for (buf = 0; buf < pkt->num_bufs; buf++) {
			if (pkt->buf[buf].data != NULL) {
#ifdef DEBUG
				if (pkt->buf[buf].data_len == 0) {
					l2tp_log(LOG_ERR, "Freeing zero length buffer %p, index %d?", 
						 pkt->buf[buf].data, buf);
				}
				USL_POISON_MEMORY(pkt->buf[buf].data, 0xe8, pkt->buf[buf].data_len);
#endif
				free(pkt->buf[buf].data);
			} else {
				break;
			}
		}
