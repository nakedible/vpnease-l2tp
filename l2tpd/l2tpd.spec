...
# Do you want to use Unix98 style ptys (FC2/3+, MDK10.1+)?
# Or do you want to use BSD style ptys (most other distributions)?
# The latter was the default for all versions before 11jdl.
%define		unix98pty 0
...
# Minimal sample configuration file for use with Openswan
Source4:	%{name}.conf
Source5:	%{name}-options.l2tpd
Source11:	%{name}-chapsecrets.sample
# Four sample Openswan configuration files
...
# Unix98 style pty allocation patch from Debian (modified)
# (http://www.l2tpd.org/patches/pty.patch)
# Currently we don't use this patch, only patches 4 xor 12.
...
# BSD style pty patch by Chris Wilson which adds /dev/ptyXX to the pppd
# parameters.
...
# fixed a syntax error and an uninitialised variable. Sorry about the remaining
...
# Patch by Marald Klein and Roger Luethi which fixes a PID file problem
Patch11:	%{name}-pidstr.patch.bz2
# Unix 98 pty patch by Nasim Mansurov / Iosif Peterfi. Code from rp-l2tp
# was used. We may have to use this patch because BSD style ptys are
# no longer supported on some Linux distributions).
Patch12:	%{name}-pty-Unix98.patch.bz2

Patch13:	%{name}-keepalive.patch
Patch14:	%{name}-gcc40.patch
Patch15:	%{name}-libgc.patch
Patch16:	%{name}-bugs.patch
...
# required for debian package build (alien)
%define         _initrddir /etc/init.d
...
%patch11 -p0 -b .pidstr
%if %{unix98pty}
%patch12 -p0 -b .unix98pty
%else
%patch4 -p0 -b .bsdpty
%endif
%patch13 -p1 -b .keepalive
%patch14 -p1 -b .gcc40
%patch15 -p1 -b .libgc
%patch16 -p1 -b .bugs
%if %{suse}
...
# Do not install init.d script because new rpmbuild croaks on files which
# are installed but not used in files -section
#%if %{suse}
...
%preun
# not needed with sbs and debian
...
# Cannot use thes in debian system
...
%changelog
* Fri Feb 4 2005 Jacco de Leeuw <jacco2@dds.nl> 0.69-11jdl
