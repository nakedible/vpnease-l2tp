...
/* Note: not used because addresses are not derived from socket anymore. */
/* Note: PMTU discovery is sensible only for connected sockets.
   Cannot do it anymore. */
/* Note: the interface MTU should be enough so that no per-socket
   MTU settings need to be chnaged. */
#else
void l2tp_tunnel_set_addresses(struct l2tp_tunnel *tunnel, struct sockaddr_in *src, struct sockaddr_in *dest)
{
	...
	if (tunnel->config.do_pmtu_discovery) {
		...
		L2TP_DEBUG(L2TP_FUNC, "%s: tunl %hu: fd=%d, set pmtu discovery w/ initial mtu %d, result=%d",
			   __FUNCTION__, tunnel->status.tunnel_id, tunnel->fd, mtu, result);
	}
}
#endif
...
struct l2tp_tunnel *l2tp_tunnel_find_by_addr(struct sockaddr_in const *peer_addr, int config_id)
{
...
#ifdef NONRANDOM_IDS /* Predictable tunnel ids above 10000. */
static uint16_t l2tp_tunnel_allocate_id(void)
{
	static uint16_t next_tunnel_id = 10001;

	if (next_tunnel_id == 0) {
		next_tunnel_id = 10001;
	}
...
static uint16_t l2tp_tunnel_allocate_id(void)
{
	...
	for (tries = 0; tries < 10; tries++) {
		if (!l2tp_test_is_no_random_ids()) {
			l2tp_make_random_vector(&tunnel_id, sizeof(tunnel_id));
		} else {
...
static int l2tp_tunnel_alloc_config_id(struct sockaddr_in const *peer_addr)
{
	...
	for (config_id = 1; config_id < L2TP_TUNNEL_MAX_CONFIGID; config_id++) {
		if (l2tp_tunnel_find_by_addr(peer_addr, config_id) == NULL) {
			return config_id;
		}
	}
				   ...
				   uint16_t error_code, char *error_string)
{
	...
	L2TP_DEBUG(L2TP_FUNC, "%s: tunl %hu: code=%hu error=%hu msg=%s", __FUNCTION__,
	...
	if (tunnel->result_code != NULL) {
		L2TP_DEBUG(L2TP_FUNC, "%s: tunl %hu: preserving current data: code=%hu error=%hu msg=%s", __FUNCTION__,
...
static int l2tp_tunnel_build_challenge(struct l2tp_tunnel *tunnel)
{
	...
	L2TP_DEBUG(L2TP_PROTOCOL, "%s: value=%s", __FUNCTION__, l2tp_buffer_hexify(tunnel->my_challenge, tunnel->my_challenge_len));
out:
	return result;
}
						 ...
						 struct l2tp_avp_challenge_response *challenge_response)
{
	...
	L2TP_DEBUG(L2TP_PROTOCOL, "%s: id=%02x secret=%s challenge=%s", __FUNCTION__,
	...
	L2TP_DEBUG(L2TP_PROTOCOL, "%s: value=%s", __FUNCTION__, l2tp_buffer_hexify(&challenge_response->value[0], 16));
}
...
#ifdef RESPONDER_ONLY
/* Note: would only be used by the initiator. */
#else
...
int l2tp_tunnel_session_add(struct l2tp_tunnel *tunnel, struct usl_list_head *list, struct usl_hlist_node *hlist, uint16_t session_id)
{
	...
	/* Chec if new session is administratively disabled */
	...
	/* If session count was zero and the cleanup timer was
...
void l2tp_tunnel_session_remove(struct l2tp_tunnel *tunnel, struct usl_list_head *list, struct usl_hlist_node *hlist)
{
	usl_list_del(list);
...
#ifdef RESPONDER_ONLY
/* Needed only when acting as initiator. */
#else
...
{	
	...
	if (tunnel->setup_timer != NULL) { /* Potentially NULL if timer allocation failed. */
		usl_timer_delete(tunnel->setup_timer);
		tunnel->setup_timer = NULL;
	}
...
{	
	...
	if (tunnel->setup_timer != NULL) { /* Just in case. */
		usl_timer_delete(tunnel->setup_timer);
		tunnel->setup_timer = NULL;
	}
...
static void l2tp_tunnel_act_cleanup(void *tun, void *session, void *arg3)
{
	...
	L2TP_DEBUG(L2TP_FUNC, "%s: %s: use_count=%d", __FUNCTION__, tunnel->fsmi.name, tunnel->status.use_count);
	...
	if (tunnel->cleanup_timer == NULL) {
		...
		/*
		 * 60 seconds is a long time to wait for tunnel to die
		 * and there is no point really. If we are closing,
		 * then there is no point waiting for the ack and if
		 * the peer is closing, then we send ack at latest
		...
		L2TP_DEBUG(L2TP_FUNC, "%s: tunl %hu: allocated cleanup_timer=%p", __FUNCTION__, tunnel->status.tunnel_id, tunnel->cleanup_timer);
	}
}
...
static void l2tp_tunnel_act_send_sccrq(void *tun, void *session, void *arg3)
{
	...
	if (l2tp_tunnel_establish_timeout != 0) {
		...
		L2TP_DEBUG(L2TP_FUNC, "%s: tunl %hu: allocated setup_timer=%p", __FUNCTION__, tunnel->status.tunnel_id, tunnel->setup_timer);
	}
...
error:
	l2tp_tunnel_log_error(tunnel, tunnel->result_code);
...
out:
	...
	if (l2tp_test_is_hold_tunnels() && tunnel->hold) {
		/* Leave tunnel context for examination. */
		L2TP_DEBUG(L2TP_FUNC, "%s: tunnel %hu down and held", __FUNCTION__, tunnel->status.tunnel_id);
		return;
	}
#endif /* L2TP_TEST */

	L2TP_DEBUG(L2TP_FUNC, "%s: tunl %hu, cleaning up tunnel", __FUNCTION__,
		   tunnel->status.tunnel_id);
...
static const struct usl_fsm_table l2tp_cce_fsm = {
...
#ifdef RESPONDER_ONLY
		/* Note: never act as an initiator. */
	...
	/*
	 * Reopening the tunnel in closing state is not required by protocol.
	 * { STATE(CLOSING),	EVENT(SCCRQ_ACCEPT),	ACTION(send_sccrp),	STATE(WAITCTLCONN) },
	 */
	{ STATE(CLOSING),	EVENT(SCCRQ_ACCEPT),	ACTION(send_stopccn),	STATE(CLOSING) },
	...
	usl_list_for_each(walk, tmp, &l2tp_tunnel_list) {
		...
		if ((tunnel2 != tunnel) &&
			...
			L2TP_DEBUG(L2TP_PROTOCOL, "%s: tunl %hu: looking at tunl %hu which is between same IP as this tunnel",
				   __FUNCTION__, tunnel->status.tunnel_id, tunnel2->status.tunnel_id);
			if (tunnel->peer.have_tiebreaker) {
				...
				if (tunnel2->config.use_tiebreaker) {
					L2TP_DEBUG(L2TP_PROTOCOL, "%s: tunl %hu: tiebreaker %s", __FUNCTION__,
						   tunnel2->status.tunnel_id, l2tp_buffer_hexify(&tunnel2->status.tiebreaker.value[0], 8));
					L2TP_DEBUG(L2TP_PROTOCOL, "%s: tunl %hu: received tiebreaker %s", __FUNCTION__,
					   ...
					   uint16_t session_id, struct l2tp_avp_desc *avps)
{
	...
	if (tunnel->peer.tunnel_id.value != 0) {
		if (avps[L2TP_AVP_TYPE_TUNNEL_ID].value->tunnel_id.value != tunnel->peer.tunnel_id.value) {
			...
			L2TP_DEBUG(L2TP_DATA, "%s: tunl %hu: mismatched tunnels: got %hu, expected %hu",
				   __FUNCTION__, tunnel->status.tunnel_id,
	...
	if (avps[L2TP_AVP_TYPE_RESULT_CODE].value != NULL) {
		...
		tunnel->peer.result_code = malloc(tunnel->peer.result_code_len + 1);
		if (tunnel->peer.result_code == NULL) {
			l2tp_stats.no_control_frame_resources++;
			goto out;
		}
		...
		if (tunnel->peer.result_code_len >= 4) {
			tunnel->peer.result_code->error_message[tunnel->peer.result_code_len - 4] = '\0';
		}
	}

       	l2tp_tunnel_log_error(tunnel, tunnel->peer.result_code);
out:
		      ...
		      uint16_t session_id, struct l2tp_packet *pkt)
{
	...
	result = l2tp_avp_message_decode(pkt->buf[0].data_len - pkt->avp_offset, 
					 pkt->buf[0].data + pkt->avp_offset, 
					 &avp_desc[0], tunnel);
	if (result == -ENOMEM) {
		l2tp_stats.no_control_frame_resources++;
		goto out;
...
void l2tp_tunnel_inc_use_count(struct l2tp_tunnel *tunnel)
{
	...
	L2TP_DEBUG(L2TP_FUNC, "%s: %s: count now %d", __FUNCTION__, tunnel->fsmi.name, tunnel->status.use_count);
}
...
{
	L2TP_DEBUG(L2TP_FUNC, "%s: tunl %hu", __FUNCTION__, tunnel->status.tunnel_id);
	if (tunnel->status.use_count != 0) {
	...
	if (tunnel->result_code != NULL) {
		/* Do not free global emergency result code! */
		if (tunnel->result_code != l2tp_tunnel_emergency_result_code) {
...
struct l2tp_tunnel *l2tp_tunnel_alloc(uint16_t tunnel_id, char *tunnel_profile_name, char *peer_profile_name, int created_by_admin, int *ret)
{
	...
	/* Default tunnel mode is LAC if we were created by management request or
	 * LNS if we were created by network request. May be overridden later 
	 * when the tunnel is associated with a peer.
	 */
...
{
	L2TP_DEBUG(L2TP_FUNC, "%s: %s", __FUNCTION__, tunnel->fsmi.name);
...
int l2tp_tunnel_xprt_create(struct l2tp_peer *peer, struct l2tp_tunnel *tunnel, struct sockaddr_in const *peer_addr)
{
...
#ifdef TWO_UDP_SOCKETS
	{
		/* Assign fd to tunnel based on tunnel mode: initiator vs. responder. */
		int local_port = ntohs(tunnel->config.local_addr.sin_port);
		switch(tunnel->status.mode) {
		case L2TP_API_TUNNEL_MODE_LAC:
			if (local_port != 1702) {
				l2tp_log(LOG_ERR, "tunnel mode LAC does not match local port: %d (expected 1702)", local_port);
				result = -EINVAL;
				goto out;
			}
			new_fd = l2tp_net_get_l2tp_client_socket();
			L2TP_DEBUG(L2TP_XPRT, "%s: setting tunnel fd to global initiator socket: %d", __FUNCTION__, new_fd);
			break;
		case L2TP_API_TUNNEL_MODE_LNS:
			if (local_port != 1701) {
				l2tp_log(LOG_ERR, "tunnel mode LNS does not match local port: %d (expected 1701)", local_port);
				result = -EINVAL;
				goto out;
			}
			new_fd = l2tp_net_get_l2tp_socket();
			L2TP_DEBUG(L2TP_XPRT, "%s: setting tunnel fd to global responder socket: %d", __FUNCTION__, new_fd);
			break;
		default:
			l2tp_log(LOG_ERR, "unsupported tunnel mode: %s", tunnel->status.mode);
			result = -EINVAL;
			goto out;
		}
	}
#else
	L2TP_DEBUG(L2TP_API, "%s: creating socket to peer %x/%hu using local %x/%hu", __FUNCTION__,
	...
	new_fd = l2tp_net_create_socket(&tunnel->config.local_addr, tunnel->config.use_udp_checksums);
#endif
	if (new_fd < 0) {
		result = -errno;
		goto error;
...
#ifdef TWO_UDP_SOCKETS
	/* No point registering this again. */
#else
...
#ifdef TWO_UDP_SOCKETS
	/* Note: must set at least the peer address (whole address) here
	 * because set_addresses is not called. */
	/* FIXME: local address is not set here. Should not be needed. */
...
static int l2tp_tunnel_param_defaults(struct l2tp_tunnel *tunnel, char *tunnel_profile_name, char *peer_profile_name)
{
	...
	tunnel->config.local_addr.sin_addr.s_addr = htonl(tunnel_profile->our_addr.s_addr);
	...
	tunnel->config.session_profile_name = strdup(peer_profile->default_session_profile_name);
	if (tunnel->config.session_profile_name == NULL) {
		result = -ENOMEM;
		goto out;
	}
	...
	tunnel->config.ppp_profile_name = strdup(peer_profile->default_ppp_profile_name);
	if (tunnel->config.ppp_profile_name == NULL) {
		result = -ENOMEM;
		goto out;
	}
...
static int l2tp_tunnel_config_set(struct l2tp_tunnel *tunnel, struct l2tp_api_tunnel_msg_data *msg, int create)
{
	...
	if (msg->flags & L2TP_API_TUNNEL_FLAG_TRACE_FLAGS) {
		tunnel->config.trace_flags = msg->trace_flags;
	}
...
bool_t l2tp_tunnel_create_1_svc(struct l2tp_api_tunnel_msg_data msg, int *retval, struct svc_req *req)
{
	...
	L2TP_DEBUG(L2TP_API, "%s: tunl %hu", __FUNCTION__, msg.tunnel_id);
	...
	if ((msg.flags & L2TP_API_TUNNEL_FLAG_CONFIG_ID) == 0) {
		...
		if (l2tp_tunnel_find_by_addr(&peer_addr, config_id)) {
			result = -L2TP_ERR_TUNNEL_ALREADY_EXISTS;
			goto out;
		}
	}
...
#ifdef TWO_UDP_SOCKETS
	/* FIXME: need to do something here if the local address
	 * is *really* required to be other than ANY. */
...
#ifdef DEBUG
	{
		...
		strcpy(dest, ip);
		L2TP_DEBUG(L2TP_API, "%s: src=%s dest=%s configId=%d secret=%s", __FUNCTION__,
...
bool_t l2tp_tunnel_delete_1_svc(u_short tunnel_id, optstring tunnel_name, int *result, struct svc_req *req)
{
	struct l2tp_tunnel *tunnel;

	L2TP_DEBUG(L2TP_API, "%s: tunl %hu", __FUNCTION__, tunnel_id);
...
#ifdef TWO_UDP_SOCKETS
	if (msg.flags & L2TP_API_TUNNEL_FLAG_USE_UDP_CHECKSUMS) {
		/* Note: this is no more a per-socket settable variable. */
		l2tp_tunnel_log(tunnel, L2TP_API, LOG_WARNING, "API: tunl %hu:: UDP checksum setting modifications disabled.", tunnel->status.tunnel_id);
	}
#else
	if (msg.flags & L2TP_API_TUNNEL_FLAG_USE_UDP_CHECKSUMS) {
...
bool_t l2tp_tunnel_get_1_svc(u_short tunnel_id, optstring tunnel_name, struct l2tp_api_tunnel_msg_data *result, struct svc_req *req)
{
	...
	if ((tunnel->result_code != NULL) && (tunnel->result_code_len > 0)) {
		...
		if (tunnel->result_code_len >= 4) {
			...
			if ((tunnel->result_code->error_message[0] != '\0') && (tunnel->result_code_len > 4)) {
...
static int l2tp_tunnel_clone(struct l2tp_tunnel *tunnel, struct l2tp_api_tunnel_msg_data *result)
{
	...
	L2TP_DEBUG(L2TP_API, "%s: tunl %hu", __FUNCTION__, tunnel->status.tunnel_id);
...
bool_t l2tp_tunnel_profile_delete_1_svc(char *profile_name, int *result, struct svc_req *req)
{
	...
	L2TP_DEBUG(L2TP_API, "%s: profile: %s", __FUNCTION__, profile_name);
