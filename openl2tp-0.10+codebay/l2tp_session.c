				    ...
				    uint16_t error_code, char *error_string)
{
	...
	if ((result_code == L2TP_AVP_RESULT_CDN_ADMIN) || (error_code != 0) || (error_string != NULL)) {
		len += 2;
...
#ifdef NONRANDOM_IDS /* Predictable session ids above 10000. */
static uint16_t l2tp_session_allocate_id(struct l2tp_tunnel *tunnel)
{
	static uint16_t next_session_id = 10001;

	if (next_session_id == 0) {
		next_session_id = 10001;
	}
				   ...
				   struct l2tp_avp_desc *avps)
{
	...
	if (avps[L2TP_AVP_TYPE_INITIAL_RCVD_LCP_CONFREQ].value != NULL) {
		if (session->peer.initial_rcvd_lcp_confreq != NULL) {
			...
			session->peer.initial_rcvd_lcp_confreq_len = 0;
		}
	...
	if (avps[L2TP_AVP_TYPE_LAST_SENT_LCP_CONFREQ].value != NULL) {
		if (session->peer.last_sent_lcp_confreq != NULL) {
			...
			session->peer.last_sent_lcp_confreq_len = 0;
		}
	...
	if (avps[L2TP_AVP_TYPE_LAST_RCVD_LCP_CONFREQ].value != NULL) {
		if (session->peer.last_rcvd_lcp_confreq != NULL) {
			...
			session->peer.last_rcvd_lcp_confreq_len = 0;
		}
	...
	if (avps[L2TP_AVP_TYPE_PROXY_AUTH_CHALLENGE].value != NULL) {
		if (session->peer.proxy_auth_challenge != NULL) {
			...
			session->peer.proxy_auth_challenge_len = 0;
		}
	...
	if (avps[L2TP_AVP_TYPE_PROXY_AUTH_RESPONSE].value != NULL) {
		if (session->peer.proxy_auth_response != NULL) {
			...
			session->peer.proxy_auth_response_len = 0;
		}
	...
	if (avps[L2TP_AVP_TYPE_CALL_ERRORS].value != NULL) {
		if (session->peer.call_errors != NULL) {
			...
			session->peer.call_errors = NULL; /* May fail below before assignment */
		}
	...
	if (avps[L2TP_AVP_TYPE_ACCM].value != NULL) {
		if (session->peer.accm != NULL) {
			...
			session->peer.accm = NULL; /* May fail below before assignment */
		}
					...
					uint16_t session_id, struct l2tp_avp_desc *avps)
{
	...
	if (avps[L2TP_AVP_TYPE_RESULT_CODE].value != NULL) {
		if (session->result_code != NULL) {
			...
			if (session->result_code != l2tp_session_emergency_result_code) {
...
static int l2tp_session_send_icrq(void *tun, void *sess)
{
	...
	bearer_type.value = 0;
	bearer_type.bits.digital = session->config.bearer_type_digital;
	bearer_type.bits.analog = session->config.bearer_type_analog;
...
static int l2tp_session_send_iccn(void *tun, void *sess)
{
	...
	framing_type.bits.async = session->config.framing_type_async;
	framing_type.bits.sync = session->config.framing_type_sync;
...
int l2tp_session_info_get(struct l2tp_session const *session, uint16_t tunnel_id, uint16_t session_id, struct l2tp_api_session_msg_data *result)
{
	...
	if (session->config.interface_name != NULL) {
		...
		char ifname[16];
		sprintf(ifname, "l2tp%hu-%hu", l2tp_tunnel_id(session->my_tunnel), session->status.session_id);
		OPTSTRING(result->interface_name) = strdup(ifname);
	}
	...
	if (session->config.initial_rcvd_lcp_confreq != NULL) {
		...
		result->initial_rcvd_lcp_confreq.initial_rcvd_lcp_confreq_val = NULL;
		result->initial_rcvd_lcp_confreq.initial_rcvd_lcp_confreq_len = 0;
	}
	...
	if (session->config.last_rcvd_lcp_confreq != NULL) {
		...
		result->last_rcvd_lcp_confreq.last_rcvd_lcp_confreq_val = NULL;
		result->last_rcvd_lcp_confreq.last_rcvd_lcp_confreq_len = 0;		
	}
	...
	if (session->config.last_sent_lcp_confreq != NULL) {
		...
		result->last_sent_lcp_confreq.last_sent_lcp_confreq_val = NULL;
		result->last_sent_lcp_confreq.last_sent_lcp_confreq_len = 0;
	}
	...
	result->peer.framing_type_sync = session->peer.framing_type.bits.sync;
	result->peer.framing_type_async = session->peer.framing_type.bits.async;
	result->peer.bearer_type_digital = session->peer.bearer_type.bits.digital;
	result->peer.bearer_type_analog = session->peer.bearer_type.bits.analog;
	...
	if (session->peer.initial_rcvd_lcp_confreq != NULL) {
		...
		result->peer.initial_rcvd_lcp_confreq.initial_rcvd_lcp_confreq_val = NULL;
		result->peer.initial_rcvd_lcp_confreq.initial_rcvd_lcp_confreq_len = 0;
	}
	...
	if (session->peer.last_rcvd_lcp_confreq != NULL) {
		...
		result->peer.last_rcvd_lcp_confreq.last_rcvd_lcp_confreq_val = NULL;
		result->peer.last_rcvd_lcp_confreq.last_rcvd_lcp_confreq_len = 0;
	}

	if (session->peer.last_sent_lcp_confreq != 0) {
		...
		result->peer.last_sent_lcp_confreq.last_sent_lcp_confreq_val = NULL;
		result->peer.last_sent_lcp_confreq.last_sent_lcp_confreq_len = 0;
	}
	...
	if (session->peer.call_errors != NULL) {
		result->peer.call_errors.crc_errors = session->peer.call_errors->crc_errors;
		result->peer.call_errors.framing_errors = session->peer.call_errors->framing_errors;
		result->peer.call_errors.hardware_overruns = session->peer.call_errors->hardware_overruns;
		result->peer.call_errors.buffer_overruns = session->peer.call_errors->buffer_overruns;
		result->peer.call_errors.timeout_errors = session->peer.call_errors->timeout_errors;
		result->peer.call_errors.alignment_errors = session->peer.call_errors->alignment_errors;;
	}
