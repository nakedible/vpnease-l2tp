...
{
	L2TP_DEBUG(L2TP_FUNC, "%s: %x/%x", __FUNCTION__, ntohl(peer->if_local_addr.s_addr), ntohl(peer->if_peer_addr.s_addr));
...
{
	peer->use_count++;
	L2TP_DEBUG(L2TP_FUNC, "%s: %x/%x: count now %d", __FUNCTION__, ntohl(peer->if_local_addr.s_addr), 
...
struct l2tp_peer_profile *l2tp_peer_profile_find_by_addr(struct in_addr peer_addr)
{
	...
	L2TP_DEBUG(L2TP_FUNC, "%s: peer_addr=%s", __FUNCTION__, inet_ntoa(peer_addr));
	usl_list_for_each(walk, tmp, &l2tp_peer_profile_list) {
		profile = usl_list_entry(walk, struct l2tp_peer_profile, list);
		if (profile->peer_addr.sin_addr.s_addr == peer_addr.s_addr) {
			L2TP_DEBUG(L2TP_FUNC, "%s: peer_addr=%s: profile %s is exact match", __FUNCTION__, 
				   inet_ntoa(peer_addr), profile->profile_name);
			return profile;
		}
		L2TP_DEBUG(L2TP_FUNC, "%s: profile %s has netmask %#x, netmask_len=%d", __FUNCTION__, 
		...
		if ((profile->peer_addr.sin_addr.s_addr & profile->netmask.s_addr) == (peer_addr.s_addr & profile->netmask.s_addr)) {
			if (profile->netmask_len > candidate_netmask_len) {
				L2TP_DEBUG(L2TP_FUNC, "%s: peer_addr=%s found candidate %s, netmask_len=%d", __FUNCTION__, 
	...
	if (candidate != NULL) {
		L2TP_DEBUG(L2TP_FUNC, "%s: peer_addr=%s: profile %s is best match", __FUNCTION__, 
			   inet_ntoa(peer_addr), candidate->profile_name);
	} else {
		L2TP_DEBUG(L2TP_FUNC, "%s: peer_addr=%s: no profile found", __FUNCTION__, 
			   inet_ntoa(peer_addr));
	}
	return candidate;
}
