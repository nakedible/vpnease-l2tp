...
static int l2tp_ppp_profile_modify(l2tp_api_ppp_profile_msg_data *msg, struct l2tp_ppp_profile *profile)
{
	...
	if (msg->flags2 & L2TP_API_PPP_PROFILE_FLAG_COMP_FLAGS) {
		profile->comp_flags = msg->comp_flags;
	}
	if (msg->flags2 & L2TP_API_PPP_PROFILE_FLAG_MPPE) {
		profile->mppe = msg->mppe;
	}
...
bool_t l2tp_ppp_profile_create_1_svc(l2tp_api_ppp_profile_msg_data msg, int *result, struct svc_req *req)
{
	...
	profile->comp_flags = l2tp_ppp_defaults->comp_flags;
	...
	profile->mppe = l2tp_ppp_defaults->mppe;
...
int l2tp_ppp_profile_get(char *name, struct l2tp_api_ppp_profile_msg_data *result)
{
	...
	result->comp_flags = profile->comp_flags;
...
void l2tp_ppp_init(void)
{
	...
	l2tp_ppp_defaults->comp_flags = L2TP_API_PPP_PROFILE_DEFAULT_COMP_FLAGS;
	...
	l2tp_ppp_defaults->mppe = L2TP_API_PPP_PROFILE_DEFAULT_MPPE;
