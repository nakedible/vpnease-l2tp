...
/* NB: this is not included in the installed pppd.h header,
 * but is defined in l2tp pppd patch. */
...
/* FIXME: should be added to system's socket.h... */
...
static CLIENT *pppol2tp_client = NULL;
static int pppol2tp_tunnel_id = 0;
static int pppol2tp_session_id = 0;
...
static int setdevname_pppol2tp(char **argv)
{
	...
	/* No sense here.. this would disable ppp compression options
	 * completely because the pppol2tp module is loaded to pppd last
	 * after the settable options. */
	/* We want to control ppp compression options separately. */
#if 0
...
static int pppol2tp_rpc_client_create(void)
{
	if (pppol2tp_client == NULL) {
		pppol2tp_client = clnt_create("localhost", L2TP_EVENT_PROG, L2TP_EVENT_VERSION, "udp");
		if (pppol2tp_client == NULL) {
...
static void pppol2tp_lcp_snoop(unsigned char *buf, int len, int incoming)
{
	...
	if (protocol <= 0x3fff) {
		...
		/* XXX: could also unset hooks here? */
		snooping = 0;
		return;
	}
	...
	while (len > 0) {
		...
		if (opt == CI_ASYNCMAP && opt_len == 0x06) {
			...
			if (got_recv_accm && got_send_accm) {
				...
				pppol2tp_send_accm_ind(send_accm, recv_accm);
				got_recv_accm = 0;
				got_send_accm = 0;
			}
		}
 ...
 */
static void log_auth_result(bool success, char *username, int namelen)
{
    char *res = "failed";
    if (success) res = "success";

    /* Truncate usernames, should never happend. */
    if (namelen > 256) {
	warn("long username truncated: it was %d characters long", namelen - 1);
	namelen = 256;
    }
    ...
    info("peer authentication %s: %s", res, username);
}
...
void plugin_init(void)
{
	...
	auth_result_peer = log_auth_result;
}
