				...
				unsigned char const *random_vector, int random_vector_len)
{
	...
	L2TP_DEBUG(L2TP_AVPDATA, "%s: avp=%hu flag_len=%hx vend=%hu, hidden data is %s", __FUNCTION__,
			 ...
			 unsigned char const *random_vector, int random_vector_len)
{
	...
	L2TP_DEBUG(L2TP_AVPDATA, "%s: pad=%d, orig_len=%d, data to hide is %s", __FUNCTION__,
	...
	new_buffer = realloc(orig_buffer, new_buffer_len);
	if (new_buffer == NULL) {
		return -ENOMEM;
	}
	...
	*buffer_len = orig_buffer_len + 2 + pad;
	...
	L2TP_DEBUG(L2TP_AVPDATA, "%s: modified data to hide is %s", __FUNCTION__,
			   ...
			   unsigned char const *random_vector, int random_vector_len)
{
	...
	L2TP_DEBUG(L2TP_AVPDATA, "%s: buffer_len=%d, data to unhide is %s", __FUNCTION__,
		   *buffer_len, l2tp_buffer_hexify(buffer + L2TP_AVP_HEADER_LEN, *buffer_len - L2TP_AVP_HEADER_LEN));

	if (*buffer_len < L2TP_AVP_HEADER_LEN) {
		L2TP_DEBUG(L2TP_AVPDATA, "%s: bad message, buffer_len=%d", __FUNCTION__, *buffer_len);
		return -EBADMSG;
	}
	...
	if (result == 0) {
		...
		if (orig_buffer_len > (*buffer_len - L2TP_AVP_HEADER_LEN)) {
			L2TP_DEBUG(L2TP_AVPDATA, "%s: bad message, orig_len=%d", __FUNCTION__, orig_buffer_len);
			return -EBADMSG;
		}
	...
	if (msg_type < L2TP_AVP_MSG_COUNT) {
		...
		while (len > 0) {
			...
			if (avp_flag_len & L2TP_AVP_HDR_HBIT) {
				...
				L2TP_DEBUG(L2TP_AVPHIDE, "%s: avp=%hu, unhidden avp now has the following data:", __FUNCTION__, avp_type);
...
#if 0 /* The specification does not require such a check and at least some
       * versions of l2tpd seem to be broken in respect to hostname avp.
       */
				
				/* check if AVP's M bit is correctly set */
				if ((l2tp_avp_info[avp_type].mandatory && (!(avp_flag_len & L2TP_AVP_HDR_MBIT))) ||
				    (!l2tp_avp_info[avp_type].mandatory && (avp_flag_len & L2TP_AVP_HDR_MBIT))) {
					l2tp_tunnel_log(tunnel, L2TP_AVP, LOG_ERR, "AVP: tunl %hu: avp_decode: bad M bit", 
							l2tp_tunnel_id(tunnel), l2tp_avp_info[avp_type].name);
					l2tp_stats.illegal_messages++;
					l2tp_stats.messages[msg_type].rx_bad++;
					goto illegal_avp;
				}
			    ...
			    struct l2tp_avp_desc *avp_data, struct l2tp_tunnel *tunnel)
{
	...
	for (avp_type = 0; avp_type < L2TP_AVP_TYPE_COUNT; avp_type++) {
		if (avp_data[avp_type].value != NULL) {
			...
			L2TP_DEBUG(L2TP_DATA, "%s: allocated %d bytes @ %p for AVP %d", __FUNCTION__,
			...
			L2TP_DEBUG(L2TP_AVPDATA, "%s: avp=%hu", __FUNCTION__, avp_type);

			pkt->buf[avp_index].data = buffer;
			pkt->buf[avp_index].data_len = buffer_len;
			...
			if (need_random_vector) {
				...
				L2TP_DEBUG(L2TP_AVPDATA, "%s: avp=%hu", __FUNCTION__, TYPE(RANDOM_VECTOR));
...
int l2tp_avp_preparse(struct l2tp_avp_hdr *first_avp, int avp_data_len, char **host_name, uint16_t *msg_type)
{
	...
	*msg_type = 0;
	...
	L2TP_DEBUG(L2TP_AVPDATA, "%s: len=%d", __FUNCTION__, avp_data_len);
	while (len > 0) {
		avp_type = ntohs(avp->type);
		...
		case L2TP_AVP_TYPE_HOST_NAME:
			...
			*host_name = strndup(&host_name_avp->string[0], avp_len);
			if (*host_name == NULL) {
...
void l2tp_avp_init(void)
{
	...
	l2tp_avp_my_host_name = calloc(1, l2tp_avp_my_host_name_len);
