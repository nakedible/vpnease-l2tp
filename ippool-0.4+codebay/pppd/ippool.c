...
static int (*old_allowed_address_hook) __P((u_int32_t addr)) = NULL;
static void (*old_ip_up_hook) __P((void)) = NULL;
static void (*old_ip_down_hook) __P((void)) = NULL;
static void (*old_ip_choose_hook) __P((u_int32_t *)) = NULL;
...
static void ippool_release_ip(void)
{
	...
	if ((ippool_addr[0].s_addr != 0) || (ippool_addr[1].s_addr != 0)) {
		...
		if (cl == NULL) {
			/* NB: cannot call fatal() here: it calls
			   exitnofity, which in turn calls us on loop.. */
			/* fatal("ippool: %s", clnt_spcreateerror(ippool_server));*/
			error("ippool: %s", clnt_spcreateerror(ippool_server));
			return;
		}
...
{
	if (old_allowed_address_hook) {
		return old_allowed_address_hook(addr);
	}
	return 1;
}
